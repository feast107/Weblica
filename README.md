...
> **Intelligent Web Application Exploration & Replaying Tool**
>
> 基于 CloakBrowser（隐蔽式 Playwright）与 Agent-in-the-Loop 智能编排引擎的 Web 应用客户端勘探复现工具。
...
# 2. 勘探目标站点（推荐 agent-mode）
python -m weblica explore <TARGET_URL> -o ./explored --depth 2 --agent-mode
...
# 4. 视觉对比（验证勘探质量）
python -m weblica compare <TARGET_URL> -d ./explored -o ./comparison
```

**关键输出目录：**
- `./explored/` — 勘探结果（HTML + assets）
- `./explored/analysis/page_001/` — 页面分析报告目录（按种类拆分的 JSON/HTML/PNG 文件，含截图、iframe、DOM 快照）
- `./explored/navigation.json` — 站点全局导航树（parent→children、depth 分组）
- `./explored/weblica-manifest.json` — 勘探元数据（页面数、资源数）
- `./explored/weblica-session.json` — **完整网络会话记录**（所有请求/响应、操作链、API 汇总）
- `./explored/.weblica-state.json` — 断点续传状态文件
- `./comparison/` — 对比截图（`original.png`, `explored.png`, `diff.png`）

---

## 功能特性

- **🕵️ 隐蔽勘探 (CloakBrowser)** — 优先使用 CloakHQ 补丁版 Chromium（58 项 C++ 级反检测补丁），自动降级到 Playwright + JS 注入方案。支持人类化行为模拟（鼠标、键盘、滚动）
...
- **📸 页面截图** — 每个勘探页面保存 `screenshot.png`（完整页面截图），便于视觉验证与对比
...
- **🔄 人机协作勘探** — 浏览器窗口在遇到登录页时**保持打开**，用户完成登录后工具自动检测并继续后续深度勘探
- **📦 深度爬取** — 支持多级页面递归勘探，自动下载并重写静态资源引用为本地路径
- **🖥️ 本地复现 (WebReplayer)** — 一键启动本地 HTTP 服务器浏览勘探结果，支持热重载
- **📸 视觉对比** — 对原始站点与勘探结果进行截图对比，量化差异
- **🎬 交互录制回放** — 录制用户在页面上的点击、输入、滚动等操作，支持在勘探站点上回放验证
...
```bash
python -m weblica compare https://example.com -d ./explored -o ./comparison
```

生成 `original.png`、`explored.png`、`diff.png`
...
```bash
# 默认启用 humanize
python -m weblica explore https://example.com

# 禁用 humanize（速度更快，但隐蔽性降低）
python -m weblica explore https://example.com --no-humanize
```
...
### 模板 A：Agent-in-the-Loop 勘探（推荐）

适用于大多数场景，Agent 在每个页面完成后监督审查（SUPERVISED 模式）。
对于复杂页面，Agent 可动态切换到步进模式（STEPPED），逐操作审批。

```
Step 1: python -m weblica explore <URL> -o ./explored --depth 2 --agent-mode
        # 或步进模式: --agent-mode --agent-stepped
Step 2: 工具自动执行 BFS 遍历，在检查点生成决策上下文
Step 3: Agent 审查页面结果，决定哪些链接值得深入、是否需要额外交互
Step 4: 读取 ./explored/analysis/page_*/ 下的分析结果
Step 5: python -m weblica compare <URL> -d ./explored -o ./comparison
Step 6: python -m weblica replay -d ./explored -p 8080
...
Step 1: python -m weblica explore <URL> -o ./explored --depth 2 --agent-mode --no-headless
Step 2: 工具检测到登录页 → 浏览器窗口保持打开
Step 3: 用户在浏览器窗口中完成登录
Step 4: 工具轮询检测到登录成功 → 自动继续 BFS 勘探
Step 5: 全部完成后启动 replay 服务器

# 传统方式：使用已有 Cookie
Step 1: python -m weblica explore <URL> -o ./explored --cookies ./cookies.json

# 传统方式：使用 Token
Step 1: python -m weblica explore <URL> -o ./explored --bearer-token <TOKEN>
```
...
```
Step 1: python -m weblica explore <URL> -o ./explored --depth 2 --agent-mode
Step 2: 读取 ./explored/navigation.json 了解站点结构与层级
Step 3: 读取 ./explored/analysis/page_001/index.json 获取概览
Step 4: 深入分析各页面：
...
适用于用户要求"验证勘探的页面能不能正常交互"。

```
Step 1: python -m weblica explore <URL> -o ./explored
Step 2: python -m weblica record <URL> --duration 30 -o session.json
Step 3: python -m weblica replay -d ./explored -p 8080
...
在已勘探的页面上执行任意点击/输入/滚动操作，捕获交互前后的 DOM 变化、截图和网络流量。如果交互触发了导航（URL 变化或 iframe src 变化），新页面会被自动分析并保存为 `analysis/page_NNN/` 条目。
...
每个勘探的页面都会生成一个独立目录，分析数据按种类拆分为多个小文件：
...
- `matched_content_page` — 该 iframe URL 对应已勘探的哪个独立页面（如果存在）
...
2. **动态内容**：SPA（React/Vue/Angular）勘探结果为静态快照，API 数据为勘探时刻的冻结状态。
